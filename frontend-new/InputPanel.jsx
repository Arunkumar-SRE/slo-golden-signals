...
//
// Props:
//   inputs      → current input values (from App.jsx state)
//   onChange    → function to update a single field
//   onCalculate → function to call when button is clicked
//   loading     → boolean — shows spinner while API is called
//   backendOk   → boolean — backend connection status
// ============================================================

// Reusable single input field
function Field({ label, name, value, onChange, hint, step = 1 }) {
  return (
...
export default function InputPanel({ inputs, onChange, onCalculate, loading, backendOk }) {
  return (
    <div style={{
      ...
      {/* Panel title */}
      <div style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em", borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: 10 }}>
        📥 Input Parameters
      </div>

      {/* ── Shared Settings ── */}
      <Section title="⚙️ Service & Window" />
      <Field label="Service Name"  name="service_name"  value={inputs.service_name}  onChange={onChange} hint="e.g. Checkout API" />
      <Field label="CUJ"           name="cuj"           value={inputs.cuj}           onChange={onChange} hint="Critical User Journey" />
      ...
      <div style={{ fontSize: 11, fontFamily: "monospace" }}>
        ...
        {backendOk === false && <span style={{ color: "#ef4444" }}>✗ Backend offline — run: python -m uvicorn main:app --reload</span>}
